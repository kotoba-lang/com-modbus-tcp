(ns modbus_tcp.main-test
  "Contract + behavioral test for the modbus_tcp-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "modbus_tcp-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'modbus_tcp.main-test)]
