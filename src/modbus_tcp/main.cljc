(ns modbus_tcp.main
  "Kotodama WASM entrypoint for the modbus_tcp clean-room actor (L4) — Clojure port.
  ...
  facts (`modbus_tcp.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "modbus_tcp")
(def tier "L4")
...
(def entity-specs
  [{:entity "MbapHeader"       :plural "mbapheaders"       :id-prefix "modbustc_mba"
    :fields [:transactionId :protocolId :length :unitId]   :required [:length]
    :coerce {:transactionId :int :protocolId :int :length :int :unitId :int} :refs {}}
   {:entity "Request"          :plural "requests"          :id-prefix "modbustc_req"
    :fields [:transactionId :protocolId :unitId :functionCode :startingAddress :quantity] :required [:functionCode :startingAddress]
    :coerce {:transactionId :int :protocolId :int :unitId :int :functionCode :int :startingAddress :int :quantity :int} :refs {}}
   {:entity "Response"         :plural "responses"         :id-prefix "modbustc_res"
    :fields [:transactionId :unitId :functionCode :byteCount] :required [:functionCode :byteCount]
    :coerce {:transactionId :int :unitId :int :functionCode :int :byteCount :int} :refs {}}
   {:entity "ExceptionResponse" :plural "exceptionresponses" :id-prefix "modbustc_exc"
    :fields [:transactionId :unitId :exceptionFunctionCode :exceptionCode] :required [:exceptionFunctionCode :exceptionCode]
    :coerce {:transactionId :int :unitId :int :exceptionFunctionCode :int :exceptionCode :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"modbus_tcp.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "modbus_tcp-compat" :tier tier :entities entities} 200])
...
(defn start! [] :modbus_tcp-compat/ready)
